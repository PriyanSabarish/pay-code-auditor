...
export default function Results({job,businessLabel,onNewReview}:{job:AuditResult;businessLabel:string;onNewReview:()=>void}) {
  ...
  return <section id="results" className="results-section fade-in" aria-label="Audit results">
    ...
    <Modal opened={!!letterCode} onClose={()=>setLetterCode(null)} title="Draft client letter" size="lg" centered><div className="sample-banner">DRAFT FOR PROFESSIONAL REVIEW</div>{letter.isPending?<p>Preparing your draft…</p>:letter.error?<Alert color="red">{letter.error.message}<Button onClick={()=>void letter.refetch()} variant="subtle">Retry</Button></Alert>:letter.data&&<><pre className="letter-text">{letter.data.subject}{'\n\n'}{letter.data.body}</pre><div className="flex gap-3 flex-wrap mb-4"><Button variant="outline" leftSection={<Copy size={15}/>} onClick={async()=>{try{await navigator.clipboard.writeText(`${letter.data.subject}\n\n${letter.data.body}`);setCopyState('Copied');}catch{setCopyState('Select text to copy');}}}>{copyState}</Button><a download className="button-secondary" href={api.letterUrl(job.audit_id,letterCode!)}>Download draft</a></div>{(letter.data.fix_steps.length>0||letter.data.catch_up_summary)&&<Accordion variant="contained"><Accordion.Item value="bookkeeper-notes"><Accordion.Control>For your records (not part of the client letter)</Accordion.Control><Accordion.Panel>{letter.data.fix_steps.length>0&&<><h3 className="letter-notes-heading">Fix steps</h3><ol className="letter-notes-steps">{letter.data.fix_steps.map((step,i)=><li key={i}>{step}</li>)}</ol></>}{letter.data.catch_up_summary&&<><h3 className="letter-notes-heading">Catch-up summary</h3><p className="reasoning-copy">{letter.data.catch_up_summary}</p></>}</Accordion.Panel></Accordion.Item></Accordion>}</>}</Modal>
  </section>;
}
