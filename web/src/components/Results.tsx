...
export default function Results({job,businessLabel}:{job:AuditResult;businessLabel:string}) {
  ...
  return <section id="results" className="results-section fade-in" aria-label="Audit results">
    <div className="section-heading"><div><div className="eyebrow">02 / YOUR FINDINGS</div><h2>{businessLabel}</h2></div>
      {complete?<a className={`button-secondary ${decision.isPending?'disabled-link':''}`} aria-disabled={decision.isPending} href={decision.isPending?undefined:api.reportUrl(job.audit_id)}><ArrowDownToLine size={16}/> Export report</a>:<span className="small-mono">{job.progress.processed_codes} OF {job.progress.total_codes} CODES</span>}
    </div>
    ...
    {verdicts.length>0&&<>
      ...
      <div className="results-toolbar"><div className="filter-pills" aria-label="Filter findings">{[['all','All codes'],['needs_review','Needs review'],['should_count','Possible shortfall'],['counts_but_shouldnt','Possible overpayment'],['correct','Correct']].map(([value,label])=><button key={value} aria-pressed={filter===value} className={filter===value?'selected':''} onClick={()=>setFilter(value)}>{label}</button>)}</div><TextInput aria-label="Search pay codes" placeholder="Find a pay code…" value={search} onChange={e=>setSearch(e.currentTarget.value)} leftSection={<Search size={15}/>}/></div>
